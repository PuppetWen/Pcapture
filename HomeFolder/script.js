...
Interceptor.attach(addresses["SSL_read"],
  {
    ...
    onLeave: function (retval) {
      ...
      console.log("Puppet1 1: " +Memory.readByteArray(this.buf, retval))
      console.log("Puppet1 2: " +this.buf)
      console.log("Puppet1 3: " +retval)
      recv(function (received_json_object) {
        console.log("Puppet1 of message: " + received_json_object);
...
Interceptor.attach(addresses["SSL_write"],
  {
    onEnter: function (args) {
      ...
      console.log("Puppet2 1: " +Memory.readByteArray(args[1], parseInt(args[2])))
      console.log("Puppet2 2: " +args[1])
      console.log("Puppet2 3: " +args[2])
      send(message, Memory.readByteArray(args[1], parseInt(args[2])));
      recv(function (received_json_object) {
        console.log("Puppet2 of message: " + received_json_object);
      }).wait(); //block execution till the message is received
    },
    onLeave: function (retval) {
    }
...
if (Java.available) {
  Java.perform(function () {
    ...
    Java.use("java.net.SocketOutputStream").socketWrite0.overload('java.io.FileDescriptor', '[B', 'int', 'int').implementation = function (fd, bytearry, offset, byteCount) {
      ...
      recv(function (received_json_object) {
          if (received_json_object.length > 1){
             NewBytearry = str.$new(received_json_object).getBytes();
          }
          else {
            NewBytearry = "1"
          }
      ...
      if (NewBytearry.length >1){
        console.log(1)
        result = this.socketWrite0(fd, NewBytearry, 0, NewBytearry.length);
      }
      else {
        console.log(2)
        result = this.socketWrite0(fd, bytearry, offset, byteCount);
      }
      return result;
    }
    Java.use("java.net.SocketInputStream").socketRead0.overload('java.io.FileDescriptor', '[B', 'int', 'int', 'int').implementation = function (fd, bytearry, offset, byteCount, timeout) {
      ...
      if (NewBytearry.length >1){
        result = this.socketRead0(fd, NewBytearry, 0, NewBytearry.length, timeout);
      }
      else {
        result = this.socketRead0(fd, bytearry, offset, byteCount, timeout);
      }
      }
      return result;
    }
